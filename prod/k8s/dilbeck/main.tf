...
config:
  ...
  PHOTOPRISM_SITE_URL: ${var.dilbeck_photoprism_site_url}
ingress:
  enabled: true
  tls: []
persistence:
  ...
  - name: originals
    hostPath:
      path: ${var.dilbeck_photoprism_originals_path}
  - name: storage
    hostPath:
      path: ${var.dilbeck_photoprism_storage_path}
  - name: imports
    hostPath:
      path: ${var.dilbeck_photoprism_imports_path}
database:
  driver: mysql
